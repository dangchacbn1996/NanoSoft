...
@objc open class PMAlertController: UIViewController {
    ...
    @IBOutlet weak var alertViewStack: UIView!
    open var ALERT_STACK_VIEW_HEIGHT : CGFloat = UIScreen.main.bounds.height < 568.0 ? 45 : 45 //if iphone 4 the stack_view_height is 40, else 62
    var animator : UIDynamicAnimator?
    
    open var textFields: [UITextField] = []
    
    @objc open var gravityDismissAnimation = false
    ...
    @objc public convenience init(title: String?, description: String?, image: UIImage?, style: PMAlertControllerStyle) {
        ...
        alertViewStack.roundCorners([.bottomLeft,.bottomRight], radius: 5.0)
...
//        alertActionStackView.addBackground(color: UIColor(hex: "EBEFF2"))
//        alertActionStackView.radi
    ...
    //MARK: - Text Fields
    @objc open func addTextField(textField:UITextField? = nil, _ configuration: (_ textField: MyTextField?) -> Void){
        let textField = textField ?? MyTextField()
        textField.delegate = self
        textField.returnKeyType = .done
        textField.font = UIFont.systemFont(ofSize: 17.0)
        textField.textAlignment = .center
        configuration (textField as? MyTextField)
