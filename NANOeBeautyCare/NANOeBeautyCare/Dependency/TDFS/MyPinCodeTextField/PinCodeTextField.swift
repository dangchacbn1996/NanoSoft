...
@IBDesignable open class PinCodeTextField: UIView {
    ...
    @IBInspectable public var updatedBorderUnderlineColor: UIColor = UIColor.clear
    ...
    private func updateUnderlines() {
        for label in labels {
            ...
            if (!highlightInputUnderline || !isInput(index)) && isPlaceholder(index) {
                ...
                underlines[index].borderWidth = 1.0
                underlines[index].cornerRadius = 5
                underlines[index].borderColor = updatedBorderUnderlineColor
            }
            else{
