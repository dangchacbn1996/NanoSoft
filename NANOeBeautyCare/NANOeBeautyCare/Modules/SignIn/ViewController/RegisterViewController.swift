...
//  NANOeBeautyCare
//
//  Created by Ngo Dang Chac on 08/09/2021.
...
class RegisterViewController: UIViewController {
    ...
    private var ipPassword: InputTextField!
    private var ipRePassword: InputTextField!
    private var ipCmnd: InputTextField!
    ...
    private var listDistrict: [ModelOptionResponseDistrictsByProvinceCodeCatalogDatum] = []
...
//        self.navigationController?.navigationItem.rightBarButtonItem = UIBarButtonItem(customView: btnRightSave)
        ...
        func setCardUser() {
            ...
            ipSex = InputTextField.droplist(title: "Giới tính", placeholder: "Chọn", listOpts: Common.sexData.map({$0.tenGioiTinh ?? "-"}), selected: { (value, index) in
                
            })
            stackUser.addArrangedSubview(UIStackView(axis: .horizontal, distribution: .fill, alignment: .fill, spacing: 12)) { (stack) in
                (stack as! UIStackView).addArrangedSubview(self.ipBirth)
                (stack as! UIStackView).addArrangedSubview(self.ipSex)
                self.ipBirth.snp.makeConstraints({
    ...
    //        ipAddress = InputTextField.normal(title: "Địa chỉ", placeholder: "Nhập địa chỉ")
            ipPassword = InputTextField.normal(title: "Mật khẩu", required: true, placeholder: "Nhập mật khẩu")
            stackUser.addArrangedSubview(ipPassword)
            
            ipRePassword = InputTextField.normal(title: "Nhập lại mật khẩu", required: true, placeholder: "Nhập lại mật khẩu")
            stackUser.addArrangedSubview(ipRePassword)
            
            ipAddress = InputTextView(title: "Địa chỉ", placeholder: "Nhập địa chỉ")
            stackUser.addArrangedSubview(ipAddress)
            
            ipCmnd = InputTextField.normal(title: "CMND/CCCD", placeholder: "Nhập CMND/CCCD")
            stackUser.addArrangedSubview(ipCmnd)
            
            ipCity = InputTextField.droplist(title: "", placeholder: "Tỉnh thành", listOpts: Common.cityData.map({$0.tenTinhThanh ?? "-"}), selected: { (city, index) in
                self.cityCode = Common.cityData[index].maTinhThanh ?? "-"
            })
            stackUser.addArrangedSubview(ipCity)
            
            ipDistrict = InputTextField.droplist(title: "", placeholder: "Quận/huyện", preload: { () -> (Void) in
                CustomerHomeService().systemDistrictsByProvinceCodeCatalog(maTinhThanh: self.cityCode) { (error, string) in
                    CommonView.alert(title: string ?? "Có lỗi xảy ra", description: "")
                } success: { (ModelOptionResponseDistrictsByProvinceCodeCatalog) in
                    self.listDistrict = ModelOptionResponseDistrictsByProvinceCodeCatalog.data ?? []
                    self.ipDistrict.actShowDrop(listData: self.listDistrict.map({$0.tenQuanHuyen ?? "-"}))
                }

            }, selected: { (string, index) in
